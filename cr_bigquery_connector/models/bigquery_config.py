    ...
    def _check_company_dataset_unique(self):
        for rec in self:
            ...
            if duplicate:
                raise UserError(
                    _(
                        "Cannot Create Multiple Configuration with Duplicate Dataset ID with Company"
                    )
                )

    def _test_bigquery_connection(self):
        try:
            credentials_dict = json.loads(self.cr_credentials_json)
            credentials = Credentials.from_service_account_info(credentials_dict)
            client = bigquery.Client(
                credentials=credentials, project=self.cr_project_id
            )
