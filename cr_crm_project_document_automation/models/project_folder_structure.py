...
class ProjectFolderStructureWizard(models.TransientModel):
    ...
    def action_create_folders(self):
        """Save structure and apply/sync folders."""
        self._save_lines()
    ...
    def action_delete_selected(self):
        """Safe deletion logic: Maps folders first, then checks for files."""
        self._save_lines()
        
        # 1. Map existing folders by name to our line IDs
        customer_data_folders = self.env['documents.document'].search([
            ('name', '=', 'Customer Data'),
            ('type', '=', 'folder')
        ])
        for cd_folder in customer_data_folders:
            self._apply_structure_to_folder(cd_folder, mapping_only=True)

        selected_lines = self.env['project.folder.structure'].search([('is_selected', '=', True)])
        if not selected_lines:
            raise UserError(_("Please select at least one line to delete."))

        # 2. Expand to all descendants for deletion candidates
        candidates = selected_lines
        for line in selected_lines:
            candidates |= line._get_descendant_lines()

        # 3. Warning Check: Only flag lines that DIRECTLY contain files.
        # Parent folders won't be listed unless they have their own direct files.
        lines_with_direct_files = candidates.filtered(lambda l: l._cr_has_direct_documents_anywhere())
        
        # We still need the recursive check for safety, but we only list the direct ones in the message.
        has_any_files = any(l._cr_has_any_documents() for l in selected_lines)
        
        if has_any_files:
            # List only the specific folders that have direct files
            names = ", ".join(lines_with_direct_files.mapped('name')) or _("sub-folders")
            message = _("The following folders contain files in some projects: %s. \n\nDo you want to proceed? Only empty folders will be deleted.") % names
        ...
        # 4. No files anywhere, safe to delete
    ...
    def _save_lines(self):
        ...
        self.env['project.folder.structure'].search([('id', 'not in', wizard_line_ids)]).unlink()
    ...
    def _perform_deletion(self, lines):
        ...
        folders = self.env['documents.document'].sudo().search([
            ('cr_project_folder_line_id', 'in', lines.ids)
        ])
        for folder in folders:
            folder._cr_safe_delete_folder()
        lines.unlink()

    def _apply_structure_to_folder(self, cd_folder, mapping_only=False):
        template_lines = self.env['project.folder.structure'].search([], order='sequence asc')
        seq_to_doc = {}
        for line in template_lines:
...
class DocumentsDocument(models.Model):
    ...
    def _cr_has_direct_documents(self):
        ...
        self.ensure_one()
        return any(d.type != 'folder' and not d.shortcut_document_id for d in self.children_ids)
