...
class IrActionsReport(models.Model):
    ...
    def _render_qweb_pdf(self, report_ref, res_ids=None, data=None):
        # Render the PDF
        pdf_content, ext = super()._render_qweb_pdf(report_ref, res_ids=res_ids, data=data)
        
        # Check if this is a custom report designer template
        report = self._get_report(report_ref)
        template = False
        if report:
            template = self.env['report.designer.template'].search([('report_action_id', '=', report.id)], limit=1)
        ...
        if not template and report_ref:
            ...
            try:
                ...
                if parts:
                    ...
                    template = self.env['report.designer.template'].browse(template_id).exists()
            except Exception:
                pass
