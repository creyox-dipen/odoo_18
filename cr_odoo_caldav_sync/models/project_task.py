...
class ProjectTask(models.Model):
    """Extends the Odoo project.task model for CalDAV synchronisation."""
    ...
    caldav_uid = fields.Char(
        ...
        copy=False,
        index=True,
        help="UUID used as the iCal UID for this task. Generated automatically.",
    )

    @api.model_create_multi
    def create(self, vals_list):
        """Override create to auto-assign a CalDAV UID to every new task.

        :param list vals_list: List of field-value dicts for new tasks.
        :return: Newly created task recordset.
        :rtype: recordset
        """
    ...
    def unlink(self):
        ...
        task_ids = self.ids
        _logger.info("[UNLINK] Triggered for task ids: %s", task_ids)
        ...
        _logger.debug(
            "[UNLINK] Found %s mapping records for these tasks.", len(all_maps)
        )

        for map_rec in all_maps:
            try:
                _logger.info(
                    '[UNLINK] CalDAV DELETE: task "%s" (id=%s) via account %s at %s',
                    map_rec.task_id.name,
                    map_rec.task_id.id,
