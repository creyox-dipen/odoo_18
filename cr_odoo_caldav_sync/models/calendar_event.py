...
class CalendarEvent(models.Model):
    ...
    def unlink(self):
        ...
        for event in self:
                ...
                if event.id == base_event.id:
                    ...
                    if g_map.account_id.id in seen_google_accounts:
                        _logger.info(
                            ...
                            g_map.id, g_map.account_id.name,
                        )
            ...
            for b_map in basic_base_maps:
                ...
                if event.id == base_event.id: 
                    ...
                    b_map.sudo().write({'google_exdates': new_exdates, 'last_odoo_write': False})
                    occ_maps = self.env['caldav.event.map'].sudo().search([
                        ('event_id', '=', event.id),
        ...
        for map_rec in all_maps:
            ...
            if map_rec.event_id and map_rec.event_id.recurrence_id and map_rec.event_id.id != map_rec.event_id.recurrence_id.base_event_id.id:
                _logger.info(
